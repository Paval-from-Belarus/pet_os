...
trait FatTrait {
    ...
    fn set_raw<T: ReadWriteSeek>(
        fat: &mut T,
        cluster: u32,
        raw_value: u32,
    ) -> io::Result<()>;
    fn set<T: ReadWriteSeek>(
        fat: &mut T,
        cluster: u32,
        value: FatValue,
    ) -> io::Result<()>;
    ...
    fn count_free<T: ReadSeek>(
        fat: &mut T,
        end_cluster: u32,
    ) -> io::Result<u32>;
}

fn read_fat<T: ReadSeek>(
    fat: &mut T,
    fat_type: FatType,
    cluster: u32,
) -> io::Result<FatValue> {
...
pub(crate) fn alloc_cluster<T: ReadWriteSeek>(
    ...
    let new_cluster =
        match find_free_cluster(fat, fat_type, start_cluster, end_cluster) {
            Ok(n) => n,
...
pub(crate) fn format_fat<T: ReadWriteSeek>(
    ...
    let end_cluster = (bytes_per_fat * BITS_PER_BYTE
        / fat_type.bits_per_fat_entry() as u64) as u32;
...
impl<T: ReadWriteSeek> Iterator for ClusterIterator<T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some(current_cluster) = self.cluster {
            self.cluster = match get_next_cluster(
                &mut self.fat,
                self.fat_type,
                current_cluster,
            ) {
...
mod tests {
    ...
    fn test_fat<T: ReadWriteSeek>(fat_type: FatType, mut cur: T) {
        ...
        assert_eq!(
            read_fat(&mut cur, fat_type, 1).unwrap(),
            FatValue::EndOfChain
        );
        assert_eq!(read_fat(&mut cur, fat_type, 4).unwrap(), FatValue::Data(5));
        assert_eq!(read_fat(&mut cur, fat_type, 5).unwrap(), FatValue::Data(6));
        assert_eq!(
            read_fat(&mut cur, fat_type, 8).unwrap(),
            FatValue::EndOfChain
        );
        assert_eq!(
            read_fat(&mut cur, fat_type, 9).unwrap(),
            FatValue::Data(0xA)
        );
        assert_eq!(
            read_fat(&mut cur, fat_type, 0xA).unwrap(),
            FatValue::Data(0x14)
        );
        ...
        assert_eq!(
            find_free_cluster(&mut cur, fat_type, 2, 0x20).unwrap(),
            0x12
        );
        assert_eq!(
            find_free_cluster(&mut cur, fat_type, 0x12, 0x20).unwrap(),
            0x12
        );
        assert_eq!(
            find_free_cluster(&mut cur, fat_type, 0x13, 0x20).unwrap(),
            0x1B
        );
        ...
        // test allocation
        assert_eq!(
            alloc_cluster(&mut cur, fat_type, None, Some(0x13), 0x1E).unwrap(),
            0x1B
        );
        assert_eq!(
            read_fat(&mut cur, fat_type, 0x1B).unwrap(),
            FatValue::EndOfChain
        );
        assert_eq!(
            alloc_cluster(&mut cur, fat_type, Some(0x1B), None, 0x1E).unwrap(),
            0x12
        );
        assert_eq!(
            read_fat(&mut cur, fat_type, 0x1B).unwrap(),
        ...
        assert_eq!(
            read_fat(&mut cur, fat_type, 0x12).unwrap(),
            FatValue::EndOfChain
        );
        ...
        assert_eq!(
            read_fat(&mut cur, fat_type, 0x16).unwrap(),
            FatValue::EndOfChain
        );
    ...
    fn test_fat12() {
        let fat: Vec<u8> = vec![
            0xF0, 0xFF, 0xFF, 0x03, 0x40, 0x00, 0x05, 0x60, 0x00, 0x07, 0x80,
            0x00, 0xFF, 0xAF, 0x00, 0x14, 0xC0, 0x00, 0x0D, 0xE0, 0x00, 0x0F,
            0x00, 0x01, 0x11, 0xF0, 0xFF, 0x00, 0xF0, 0xFF, 0x15, 0x60, 0x01,
            0x19, 0x70, 0xFF, 0xF7, 0xAF, 0x01, 0xFF, 0x0F, 0x00, 0x00, 0x70,
            0xFF, 0x00, 0x00, 0x00,
        ];
    ...
    fn test_fat16() {
        let fat: Vec<u8> = vec![
            0xF0, 0xFF, 0xFF, 0xFF, 0x03, 0x00, 0x04, 0x00, 0x05, 0x00, 0x06,
            0x00, 0x07, 0x00, 0x08, 0x00, 0xFF, 0xFF, 0x0A, 0x00, 0x14, 0x00,
            0x0C, 0x00, 0x0D, 0x00, 0x0E, 0x00, 0x0F, 0x00, 0x10, 0x00, 0x11,
            0x00, 0xFF, 0xFF, 0x00, 0x00, 0xFF, 0xFF, 0x15, 0x00, 0x16, 0x00,
            0x19, 0x00, 0xF7, 0xFF, 0xF7, 0xFF, 0x1A, 0x00, 0xFF, 0xFF, 0x00,
            0x00, 0x00, 0x00, 0xF7, 0xFF, 0x00, 0x00, 0x00, 0x00,
        ];
    ...
    fn test_fat32() {
        let fat: Vec<u8> = vec![
            0xF0, 0xFF, 0xFF, 0x0F, 0xFF, 0xFF, 0xFF, 0x0F, 0xFF, 0xFF, 0xFF,
            0x0F, 0x04, 0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x06, 0x00,
            0x00, 0x00, 0x07, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0xFF,
            0xFF, 0xFF, 0x0F, 0x0A, 0x00, 0x00, 0x00, 0x14, 0x00, 0x00, 0x00,
            0x0C, 0x00, 0x00, 0x00, 0x0D, 0x00, 0x00, 0x00, 0x0E, 0x00, 0x00,
            0x00, 0x0F, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x11, 0x00,
            0x00, 0x00, 0xFF, 0xFF, 0xFF, 0x0F, 0x00, 0x00, 0x00, 0x00, 0xFF,
            0xFF, 0xFF, 0x0F, 0x15, 0x00, 0x00, 0x00, 0x16, 0x00, 0x00, 0x00,
            0x19, 0x00, 0x00, 0x00, 0xF7, 0xFF, 0xFF, 0x0F, 0xF7, 0xFF, 0xFF,
            0x0F, 0x1A, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0x0F, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF7, 0xFF, 0xFF, 0x0F, 0x00,
