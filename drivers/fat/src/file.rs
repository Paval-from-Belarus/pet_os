...
impl<'a, T: ReadWriteSeek> File<'a, T> {
    ...
    pub fn truncate(&mut self) -> io::Result<()> {
        ...
        if self.offset > 0 {
            ...
            self.fs
                .truncate_cluster_chain(self.current_cluster.unwrap()) // SAFE
        } else {
    ...
    pub(crate) fn abs_pos(&self) -> Option<u64> {
        ...
        match self.current_cluster {
            Some(n) => {
                ...
                let offset_in_fs = self.fs.offset_from_cluster(n)
                    + u64::from(offset_in_cluster);
                Some(offset_in_fs)
            }
            None => None,
        }
...
impl<'a, T: ReadWriteSeek> Read for File<'a, T> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        let bytes_left_in_file =
            self.bytes_left_in_file().unwrap_or(bytes_left_in_cluster);
        ...
        let offset_in_fs = self.fs.offset_from_cluster(current_cluster)
            + (offset_in_cluster as u64);
        let read_bytes = {
...
impl<'a, T: ReadWriteSeek> Write for File<'a, T> {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        let offset_in_cluster = self.offset % cluster_size;
        let bytes_left_in_cluster = (cluster_size - offset_in_cluster) as usize;
        let bytes_left_until_max_file_size =
            (MAX_FILE_SIZE - self.offset) as usize;
        ...
        let current_cluster = if self.offset % cluster_size == 0 {
            ...
            match next_cluster {
                ...
                None => {
                    ...
                    let new_cluster = self
                        .fs
                        .alloc_cluster(self.current_cluster, self.is_dir())?;
            ...
            match self.current_cluster {
                Some(n) => n,
                None => {
                    panic!("Offset inside cluster but no cluster allocated")
                }
            }
        };
    ...
    fn seek(&mut self, pos: SeekFrom) -> io::Result<u64> {
        let mut new_pos = match pos {
            ...
            SeekFrom::End(x) => {
                let size = self
                    .size()
                    .expect("cannot seek from end if size is unknown")
                    as i64;
                size + x
            }
        };
        ...
        let cluster_count =
            (self.fs.clusters_from_bytes(new_pos as u64) as i32 - 1) as isize;
        let old_cluster_count = (self.fs.clusters_from_bytes(self.offset as u64)
            as i32
            - 1) as isize;
                    ...
                    for i in 0..cluster_count {
                        cluster = match iter.next() {
                            ...
                            None => {
                                ...
                                new_pos =
                                    self.fs.bytes_from_clusters((i + 1) as u32)
                                        as u32;
                                break;
                            }
                        };
