...
impl ShortName {
    ...
    pub(crate) fn new(raw_name: &[u8; 11]) -> Self {
        ...
        let total_len = if ext_len > 0 {
            name[name_len] = b'.';
            name[name_len + 1..name_len + 1 + ext_len]
                .copy_from_slice(&raw_name[8..8 + ext_len]);
    ...
    fn eq_ignore_case(
        &self,
        name: &str,
        oem_cp_converter: &OemCpConverter,
    ) -> bool {
        // Strip non-ascii characters from short name
...
impl DirFileEntryData {
    ...
    pub(crate) fn first_cluster(&self, fat_type: FatType) -> Option<u32> {
        ...
        let n =
            ((first_cluster_hi as u32) << 16) | self.first_cluster_lo as u32;
        if n == 0 {
            None
        } else {
            Some(n)
    ...
    pub(crate) fn set_first_cluster(
        &mut self,
        cluster: Option<u32>,
        fat_type: FatType,
    ) {
...
impl DirLfnEntryData {
    ...
    pub(crate) fn copy_name_from_slice(
        &mut self,
        lfn_part: &[u16; LFN_PART_LEN],
    ) {
...
impl DirEntryEditor {
    ...
    pub(crate) fn flush<T: ReadWriteSeek>(
        &mut self,
        fs: &FileSystem<T>,
    ) -> io::Result<()> {
        if self.dirty {
...
impl<'a, T: ReadWriteSeek> DirEntry<'a, T> {
    ...
    pub(crate) fn eq_name(&self, name: &str) -> bool {
        ...
        let self_name_lowercase_iter =
            self_name.chars().flat_map(|c| c.to_uppercase());
        let other_name_lowercase_iter =
            name.chars().flat_map(|c| c.to_uppercase());
        let long_name_matches =
            self_name_lowercase_iter.eq(other_name_lowercase_iter);
        let short_name_matches = self
            .short_name
...
mod tests {
    ...
    fn short_name_with_ext() {
        ...
        assert_eq!(
            ShortName::new(&raw_short_name).eq_ignore_case(
                "\u{FFFD}OOK AT.M \u{FFFD}",
                &LOSSY_OEM_CP_CONVERTER
            ),
            true
        );
    }
    ...
    fn short_name_05_changed_to_e5() {
        ...
        assert_eq!(
            ...
            [
                0xE5, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, b'.', 0x05,
                0x05, 0x05
            ]
        );
    }
    ...
    fn lowercase_short_name() {
        ...
        assert_eq!(
            raw_entry
                .lowercase_name()
                .to_string(&LOSSY_OEM_CP_CONVERTER),
            "foo.rs"
        );
