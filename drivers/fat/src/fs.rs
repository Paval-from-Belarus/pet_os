...
use file::File;
use table::{
    alloc_cluster, count_free_clusters, format_fat, read_fat_flags,
    ClusterIterator, RESERVED_FAT_ENTRIES,
};
...
impl FsInfoSector {
    ...
    fn deserialize<T: Read>(rdr: &mut T) -> io::Result<FsInfoSector> {
        ...
        if lead_sig != Self::LEAD_SIG {
            return Err(Error::new(
                ErrorKind::Other,
                "invalid lead_sig in FsInfo sector",
            ));
        }
        ...
        if struc_sig != Self::STRUC_SIG {
            return Err(Error::new(
                ErrorKind::Other,
                "invalid struc_sig in FsInfo sector",
            ));
        }
        ...
        if trail_sig != Self::TRAIL_SIG {
            return Err(Error::new(
                ErrorKind::Other,
                "invalid trail_sig in FsInfo sector",
            ));
        }
        Ok(FsInfoSector {
            free_cluster_count,
    ...
    fn serialize<T: Write>(&self, wrt: &mut T) -> io::Result<()> {
        ...
        wrt.write_u32::<LittleEndian>(
            self.free_cluster_count.unwrap_or(0xFFFFFFFF),
        )?;
        wrt.write_u32::<LittleEndian>(
            self.next_free_cluster.unwrap_or(0xFFFFFFFF),
        )?;
...
impl FsOptions {
    ...
    pub fn oem_cp_converter(
        mut self,
        oem_cp_converter: &'static OemCpConverter,
    ) -> Self {
...
impl<T: ReadWriteSeek> FileSystem<T> {
    ...
    pub fn volume_label(&self) -> String {
        ...
        let char_iter =
            volume_label_iter.map(|c| self.options.oem_cp_converter.decode(c));
    ...
    pub fn read_volume_label_from_root_dir(
        ...
        let volume_label_opt =
            self.read_volume_label_from_root_dir_as_bytes()?;
        if let Some(volume_label) = volume_label_opt {
            ...
            let char_iter = volume_label_iter
                .map(|c| self.options.oem_cp_converter.decode(c));
            // Build string from character iterator
    ...
    pub(crate) fn cluster_iter<'b>(
        &'b self,
        cluster: u32,
    ) -> ClusterIterator<DiskSlice<FsIoAdapter<'b, T>>> {
    ...
    fn flush_fs_info(&self) -> io::Result<()> {
        ...
        if self.fat_type == FatType::Fat32 && fs_info.dirty {
            let mut disk = self.disk.borrow_mut();
            disk.seek(SeekFrom::Start(
                self.offset_from_sector(self.bpb.fs_info_sector as u32),
            ))?;
...
fn fat_slice<T: ReadWriteSeek>(
    ...
    let (fat_first_sector, mirrors) = if mirroring_enabled {
        ...
        let fat_first_sector =
            (bpb.reserved_sectors()) + active_fat * sectors_per_fat;
        (fat_first_sector, 1)
    };
...
impl<'a, T: Write + Seek> Write for DiskSlice<T> {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        let offset = self.begin + self.offset;
        let write_size =
            cmp::min((self.size - self.offset) as usize, buf.len());
        if write_size == 0 {
            return Ok(0);
        }
        // Write data
        for i in 0..self.mirrors {
            self.inner
                .seek(SeekFrom::Start(offset + i as u64 * self.size))?;
...
pub(crate) static LOSSY_OEM_CP_CONVERTER: LossyOemCpConverter =
    LossyOemCpConverter { _dummy: () };

pub(crate) fn write_zeros<T: ReadWriteSeek>(
    mut disk: T,
    mut len: u64,
) -> io::Result<()> {
...
fn write_zeros_until_end_of_sector<T: ReadWriteSeek>(
    mut disk: T,
    bytes_per_sector: u16,
) -> io::Result<()> {
    let pos = disk.seek(SeekFrom::Current(0))?;
    let total_bytes_to_write =
        bytes_per_sector as u64 - (pos % bytes_per_sector as u64);
...
pub fn format_volume<T: ReadWriteSeek>(
    ...
    let (boot, fat_type) =
        format_boot_sector(&options, total_sectors, bytes_per_sector)?;
    boot.validate()?;
    ...
    if boot.bpb.is_fat32() {
        ...
        disk.seek(SeekFrom::Start(
            boot.bpb.bytes_from_sectors(boot.bpb.fs_info_sector()),
        ))?;
        ...
        // backup boot sector
        disk.seek(SeekFrom::Start(
            boot.bpb.bytes_from_sectors(boot.bpb.backup_boot_sector()),
        ))?;
        boot.serialize(&mut disk)?;
        write_zeros_until_end_of_sector(&mut disk, bytes_per_sector)?;
    }
    ...
    if fat_type == FatType::Fat32 {
        ...
        let first_data_sector =
            reserved_sectors + sectors_per_all_fats + root_dir_sectors;
        let root_dir_first_sector = first_data_sector
            + boot.bpb.sectors_from_clusters(
                root_dir_first_cluster - RESERVED_FAT_ENTRIES,
            );
        let root_dir_pos = boot.bpb.bytes_from_sectors(root_dir_first_sector);
        disk.seek(SeekFrom::Start(root_dir_pos))?;
    ...
    if let Some(volume_label) = options.volume_label {
        disk.seek(SeekFrom::Start(root_dir_pos))?;
        let volume_entry =
            DirFileEntryData::new(volume_label, FileAttributes::VOLUME_ID);
